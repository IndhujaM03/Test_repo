...
getThumbnailGradient(): string {
  ...
  const fallbackDark = '#1e40af';

  if (!this.selectedTool || !this.selectedTool.categoryColor) {
    return `linear-gradient(135deg, ${fallbackPrimary}, ${fallbackDark})`;
  }
  ...
  const darkened = this.darkenColorSafe(color, 20);
  return `linear-gradient(135deg, ${color}, ${darkened})`;
}
...
private darkenColorSafe(color: string, percent: number): string {
